...
type fieldsContainer interface {
	find(column string, value reflect.Value) any
}
...
func (s fieldsContainerSlice) find(column string, value reflect.Value) any {
	for _, v := range s {
		if v.dbTag == column {
...
func (s fieldsContainerMap) find(column string, value reflect.Value) any {
	idx, ok := s[column]
	if !ok {
		return emptyScanObj
	}
...
func (f *fieldPath) eface(t reflect.Value) any {
	next := f
